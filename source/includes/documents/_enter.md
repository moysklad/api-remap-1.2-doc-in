...
| **sum** | Float                                             | `=` `!=` `<` `>` `<=` `>=` | Stock Adjustment Total in paise<br>`+Required when replying` `+Read-only` `+Change-handler` |
...
|---------|-------|------- |
| **sum** | Float | Stock Adjustment Total in paise<br>`+Required when replying` `+Update-provider` |
...
| **overhead** | Int                                                | Overhead expenses. [Learn more](../documents/#transactions-stock-adjustment-stock-adjustment-overhead-expenses). If Stock Adjustment items are not set, then no overhead expenses can be set<br>`+Required when replying` `+Read-Only` |
